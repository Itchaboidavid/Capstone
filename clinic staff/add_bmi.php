                ...
                <div class="card bg-body-secondary">
                    ...
                    <div class="card-body">
                        <form action="" method="POST" class="needs-validation w-100 row g-1" validate>
                            ...
                            <h2>HFA</h2>
                            <div class="form-floating mb-3 col-6 d-inline-block">
                                <input type="text" name="mother_height" id="mother_height" placeholder="mother_height" class="form-control bg-body-tertiary" required />
                                <label for="mother_height">Mother's Height(cm)</label>
                                <div class="valid-feedback ps-1">Great!</div>
                                <div class="invalid-feedback ps-1"> Please enter student mother's height.</div>
                            </div>
                            <div class="form-floating mb-3 col-6 d-inline-block">
                                <input type="text" name="father_height" id="father_height" placeholder="father_height" class="form-control bg-body-tertiary" required />
...
if (isset($_POST['submit'])) {
    ...
    if ($row["sex"] == "M") {
        $parent_height + 7.6;
        $hfa = $parent_height;
        $float_hfa = number_format((float)$hfa, 2, '.', '');
    } elseif ($row["sex"] == "F") {
        $parent_height - 7.6;
        $hfa = $parent_height;
        $float_hfa = number_format((float)$hfa, 2, '.', '');
    ...
    $update = "UPDATE `student` SET `height`='$height',`weight`='$weight',`height2`='$height2',`bmi`='$bmi',`bmi_category`='$bmi_category',`mother_height`='$mother_height',`father_height`='$father_height',`hfa`='$float_hfa' WHERE id = $id";
    $result = mysqli_query($conn, $update);
    echo ("<script>location.href = 'student_table.php?msg=BMI & HFA added successfully!';</script>");
    exit();
}
?>